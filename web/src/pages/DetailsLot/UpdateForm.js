...
function UpdateForm(props) {
  ...
  function onFinish(values){
    ...
    exeRequest("/categories", "PUT", values, function (response) {
      if (response.statusCode == STATUS_CODE_REST_API[1].code) {
        notification.success({
          message: 'Mediassur App',
          description: 'Categorie enregistrée avec succès'
        });
        ...
        const description = response.statusCode == STATUS_CODE_REST_API[3].code ? 
          response.message : "Erreur lors de l'enregistrement de la categorie"
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item
                ...
                initialValue={currentItem.libelle}
              >
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item
                name="icon"
                label="Icon"
                rules={[{ required: true, message: "Veuillez saisir l'icon" }]}
                initialValue={currentItem.icon}
              >
                <Input placeholder="Veuillez saisir l'icon" />
              </Form.Item>
            </Col>
          </Row>
